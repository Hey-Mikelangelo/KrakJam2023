...
public static class ColorExtensions
{
    ...
    public static float GetSimilarityCoeficentHSV(this Color color, Color otherColor, float hMult, float sMult, float vMult)
    {
        Color.RGBToHSV(color, out var h, out var s, out var v);
        Color.RGBToHSV(otherColor, out var otherH, out var otherS, out var otherV);
    ...
    public static float GetSimilarityCoeficentRGB(this Color color, Color otherColor, float rMutl, float gMult, float bMult)
    {
        float hVal = (1 - Mathf.Abs(color.r - otherColor.r) * rMutl);
        float sVal = (1 - Mathf.Abs(color.g - otherColor.g) * gMult);
        float vVal = (1 - Mathf.Abs(color.b - otherColor.b) * bMult);
        float a;
        return (hVal + sVal + vVal) / (3);
